...
//
// A triagem por nível foi APOSENTADA em 2026-08-25 (decisão dele, rodada 57):
// o raio-X resolve o mesmo problema pelo lado oposto — acende o que está
// ACIMA do nível em vez de marcar o que está abaixo. O bloco (~570 linhas)
// vive no histórico do git; o que ficou gravado no BookDB é inócuo.
...
function lerDesfazerTriagem() {
  ...
  if (!u) return
  // Só 'ignorar' empilha desfazer hoje (o ramo 'conheco' saiu com a triagem).
  if (typeof markIgnoredWord === 'function') markIgnoredWord(u.w, false)
  ...
  if (a && u.n) {
    ...
    // Espelho exato do que _lerTirarDaLista fez com 'ignorar': devolve as
    // ocorrências ao total — desfazer pela metade deixaria a cobertura mentindo.
    a.total += u.n
...
function _lerTirarDaLista(palavra, tipo) {
  ...
  // "Ignorar" (nome próprio) sai da conta dos dois lados: não é vocabulário
  // a aprender nem vocabulário que você sabe. 'estudo' não mexe em contador.
  if (tipo === 'ignorar') a.total = Math.max(1, a.total - item.n)
  a.cobertura = a.total ? Math.min(1, a.conhecidas / a.total) : 0
  _lerPintarFerramentas()
}

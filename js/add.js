    ...
    const vocabRefHTML = item.vocab_ref?.length
      ? `<div style="font-size:var(--fs-sm);color:var(--text2);margin-top:5px;display:flex;flex-wrap:wrap;gap:8px">`
        + item.vocab_ref.map(v => v.expr
          ? `<span>${ic('target','ic-sm')} ${esc(v.expr)}${v.type ? ` <span style="font-size:var(--fs-3xs);background:rgba(var(--primary-rgb),.15);color:var(--primary);border-radius:3px;padding:1px 4px">${esc(v.type)}</span>` : ''}</span>`
          : '').filter(Boolean).join(' ')
        + '</div>'
      : ''
    return `
